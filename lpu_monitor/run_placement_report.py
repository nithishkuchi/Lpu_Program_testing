"""
run_placement_report.py

Records: Placement information for ALL programmes.
"""
...
OUT_XLSX = "records_placement.xlsx"
...
def get_all_programmes(proglist_path):
    """
    Read all programme codes and names from the programme list.

    Unlike the LPUNEST report, this does NOT exclude PhD programmes,
    because this report is intended to cover all programmes.
    """
    ...
    for row in range(1, sheet.nrows):
        code = sheet.cell_value(row, code_col)
...
def extract_metrics(details):
    """
    Extract useful numeric information from the placement details.

    This is intentionally tolerant because the API's HTML
    can contain different metrics.
    """
    ...
    if lakh_match:
        metrics["package_lakh"] = lakh_match.group(1)
    ...
    if plus_match:
        metrics["value"] = plus_match.group(1)

    return metrics


def parse_records(api_response, programme_code, programme_name):
    """
    Convert the raw endpoint response into flat records.
    """
        ...
        records.append(
            {
                ...
                "Metrics": ", ".join(
                    f"{key}: {value}"
                    for key, value in extract_metrics(details).items()
                ),
...
async def main():
    ...
    print("=" * 70)
    print("PLACEMENT RECORDS REPORT")
    print("=" * 70)
        ...
        for index, programme in enumerate(
            ...
            try:
                ...
                placement_records = [
                    ...
                    if record["Category"].strip().lower()
                    == "placements"
                ]
    ...
    print()
    print("=" * 70)
    print("PLACEMENT REPORT COMPLETE")
    print("=" * 70)
    print(
        f"Total placement records : {len(df)}"
    )
    print(
